...
def assign_groups_and_weights(factor, direction, num, factor_name):
    def process_group(df):
        df['factor_value'] += np.random.normal(0, 1e-12, len(df))
        if direction > 0:
            df = df.sort_values(by='factor_value')
        elif direction < 0:
            df = df.sort_values(by='factor_value', ascending=False)
        df = df.head(num)
        df['weight'] = 1 / len(df) if len(df) > 0 else 0
        return df

    result = factor.groupby('date').apply(process_group).reset_index(drop=True)
    result = pd.merge(result, datelist, on=['date'], how = 'left')
    return result
...
class Positions2Nav(object):
    ...
    def update_portfolio_navs(self, isupdate, sample_symbol, **kwargs):
        '''This function updates the g_positions for factors and factor portfolios and writes to database.'''
        # config
        logger = self.logger
        logger.info("----------------Start %s.%s().----------------" %(self.__class__.__name__,inspect.stack()[0][3]))
        ...
        if portfolio_info_eff.shape[0]>0:
            ...
            cnavs = pd.DataFrame() #may not be useful
...
if __name__=='__main__':
    ...
    stock_status = stock_status[(stock_status['trade_status'] == 1) & (stock_status['is_ST'] == 0)] #(stock_status['is_new_stock'] == 0) &

    '''
    datelist = pd.read_parquet(datadir + '/config/datelist.pq')
    datelist['date'] = pd.to_datetime(datelist['date'])
    trading_days = pd.read_parquet(datadir + '/market_data/tradying_days.pq')
    trading_days['position_adjust_date'] = trading_days['date'].shift(-1)
    datelist = trading_days.merge(datelist, on=['date'], how='inner')
    '''
    datelist = pd.read_parquet(REBALANCE_DATES)

    def main_process(sample_symbol, num_stocks, datelist):
        ...
        port_info = pd.read_excel(portfolio_info_path(), index_col = None)
        factors_dict = pd.read_excel(factors_dict_path(), index_col = None)

        port_info = port_info[port_info['stock_area'] == sample_symbol]
        if port_info.empty:
            print(f'{sample_symbol}: no portfolio in {portfolio_info_path()}, skipped')
            return

        try:
            g_positions_old = pd.read_parquet(datadir + f'/g_positions/g_positions{neu}/g_positions_{sample_symbol}.pq')
            last_date = g_positions_old['date'].max()
        except:
            g_positions_old = pd.DataFrame(columns=['portfolio_id', 'date', 'instrument', 'weight'])
            last_date = pd.to_datetime(start_date)

        try:
            navs_old = pd.read_parquet(datadir + f'/navs/navs{neu}/navs_{sample_symbol}.pq')
            navs_old.to_parquet(datadir + '/navs/navs.pq')
        except:
            print('')

        if pd.to_datetime(end_date) > datelist[datelist['position_adjust_date'] >= last_date].iloc[0]['date']:

            for factor_name in port_info['factor_name'].unique():
                print(f'{factor_name} begin!')
                try:
                    #factor_table = factors_dict.loc[factors_dict['factor_name'] == factor_name, 'factor_table'].values[0]
                    #factor_id = factors_dict.loc[factors_dict['factor_name'] == factor_name, 'factor_id'].values[0]
                ...
                port_id = port_info.loc[port_info['factor_name'] == factor_name, 'portfolio_id'].values[0]
                corr = port_info.loc[port_info['factor_name'] == factor_name, 'factor_direction'].values[0]
                ...
                if corr > 0:
                    ...
                    continue
                try:
                    #factor_subset = factor_subset[factor_subset['factor_name'] == factor_name][['date', 'stock_id', 'factor_value']]
                    if len(factor_subset) == 0:
                        continue
                except:
                    continue

                factor_subset = factor_subset[factor_subset['date'] >= last_date]
                factor_subset = factor_subset[['date', 'stock_id', 'factor_value']]
                factor_subset['date'] = pd.to_datetime(factor_subset['date'])
                df_subset = pd.merge(factor_subset, stock_status_sample, on=['date', 'stock_id'], how='left')
                ...
                result_df = assign_groups_and_weights(df_subset, direction, num_stocks, factor_name)
                result_df = result_df[['position_adjust_date', 'stock_id', 'weight']].rename(columns={'position_adjust_date': 'date', 'stock_id': 'instrument'}).reset_index(drop=True).dropna(subset='date')
                result_df['portfolio_id'] = port_id
                result_df['portfolio_id'] = result_df['portfolio_id'].astype(np.int64)
                result_df = result_df[['portfolio_id', 'date', 'instrument', 'weight']]

                g_positions_old = pd.concat([g_positions_old, result_df], axis=0, ignore_index=True)
                g_positions_old.sort_values(by=['portfolio_id', 'date'], inplace=True)
                g_positions_old = g_positions_old.reset_index(drop=True)
        ...
        g_positions_old.to_parquet(ensure_parent(datadir + '/g_positions/g_positions_smart_beta.pq'))
        g_positions_old.to_parquet(ensure_parent(datadir + f'/g_positions/g_positions{neu}/g_positions_{sample_symbol}.pq'))
        ...
        obj_positions2nav.update_portfolio_navs(isupdate_nav, sample_symbol=sample_symbol)
        navs = pd.read_parquet(datadir + '/navs/navs.pq')
        navs.to_parquet(ensure_parent(datadir + f'/navs/navs{neu}/navs_{sample_symbol}.pq'))


    # used for universes with weighting='float_mv' (originally only 'kc'): float_mv-weighted, capped at WEIGHT_CAP
    def main_process_kc(sample_symbol, num_stocks, datelist):
        ...
        if pd.to_datetime(end_date) > datelist[datelist['position_adjust_date'] > last_date].iloc[0]['date']:
            ...
            for factor_name in port_info['factor_name'].unique():
                ...
                result_df = result_df.groupby(['date']).apply(calculate_weights).reset_index(drop=True)
                result_df = result_df.sort_values(by=['date', 'stock_id']).reset_index(drop=True)
    ...
    for universe, params in UNIVERSES.items():
        if params['weighting'] == 'float_mv':
            main_process_kc(universe, params['top_n'], datelist)
        else:
            main_process(universe, params['top_n'], datelist)
